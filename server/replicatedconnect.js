...
const replicatedDb = new sqlite3.Database(
  "./db/replicatedDazzle.db",
...
mainDb.each(
  ...
  (err, row) => {
    if (err) console.error(err.message);
      ...
      replicatedDb.run(
        "INSERT OR IGNORE INTO items (id, name, price, imgURL, size) VALUES (?, ?, ?, ?, ?)",
...
mainDb.each(
  ...
  (err, row) => {
    if (err) console.error(err.message);
      ...
      replicatedDb.run(
        "INSERT OR IGNORE INTO item_sizes (id, name, size, qty) VALUES (?, ?, ?, ?)",
        [row.id, row.name, row.size, row.qty],
        (err) => {
          if (err) console.error(err.message);
        }
      );
...
mainDb.each(
  ...
  (err, row) => {
    if (err) console.error(err.message);
      ...
      replicatedDb.run(
        "INSERT OR IGNORE INTO customer (id, name, orderNum) VALUES (?, ?, ?)",
        [row.id, row.name, row.orderNum],
        (err) => {
          if (err) console.error(err.message);
        }
      );
